...
async function fetchPopular() {
  ...
  return cards.filter((c) => c.visible !== false).slice(0, 8).map((c, i) => ({
    id: c.id || `pop-${i}`,
...
// IMPORTANT: just arrived uses only /api/just-arrived
  ...
  return (
    <article className="relative rounded-2xl border border-neutral-200 bg-white shadow-sm transition duration-200 hover:shadow-2xl hover:-translate-y-0.5">
      <div className="relative p-4">
        {item.discount > 0 && <span className="absolute left-4 top-4 rounded-md bg-green-100 text-green-700 text-xs font-semibold px-2 py-1">-{item.discount}%</span>}
        <button onClick={onToggleWish} className="absolute right-4 top-4 h-9 w-9 grid place-items-center rounded-full bg-white/90 ring-1 ring-neutral-200 hover:bg-white" aria-label="Add to wishlist" type="button">
  ...
  return (
    <section className="mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center">
            <button type="button" onClick={onArrowLeft} className="rounded-md bg-neutral-100 hover:bg-neutral-200 p-2" aria-label="Previous"><ChevronLeft size={18} /></button>
            <button type="button" onClick={onArrowRight} className="ml-2 rounded-md bg-neutral-100 hover:bg-neutral-200 p-2" aria-label="Next"><ChevronRight size={18} /></button>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        tabIndex={0}
        onKeyDown={(e) => { if (e.key === "ArrowRight") { e.preventDefault(); onArrowRight(); } if (e.key === "ArrowLeft") { e.preventDefault(); onArrowLeft(); } }}
        onWheel={(e) => { if (Math.abs(e.deltaY) > Math.abs(e.deltaX)) { e.preventDefault(); e.currentTarget.scrollBy({ left: e.deltaY, behavior: "smooth" }); } }}
        className="
          grid grid-flow-col
...
export default function PopularAndArrived() {
  ...
  const popLeft  = () => scrollByOne(popRef.current,  firstPopRef.current,  "left");
  const popRight = () => scrollByOne(popRef.current,  firstPopRef.current,  "right");
  const arrLeft  = () => scrollByOne(arrRef.current,  firstArrRef.current,  "left");
  const arrRight = () => scrollByOne(arrRef.current,  firstArrRef.current,  "right");
