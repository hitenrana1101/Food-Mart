  ...
  export default function AdminTrendingSimple() {
    ...
    // Order handler: checks qty and shows availability
    const orderProduct = (id, qty) => {
              ...
              <li key={r.id} className="rounded border border-neutral-200 bg-white p-2">
                <div className="flex items-start gap-2">
                  ...
                  <div className="grid flex-1 grid-cols-1 gap-2">
                    ...
                    {/* Second row (controls) */}
                    <div className="grid grid-cols-2 sm:grid-cols-[160px_repeat(5,minmax(0,1fr))_auto_auto_auto] gap-2">
                      <select
                        value={r.category}
            ...
            {(filter === "ALL" ? rows : rows.filter((r) => r.category === filter))
              ...
              .map((p) => (
                <article key={p.id} className="relative rounded-2xl bg-white ring-1 ring-neutral-100 shadow-[0_8px_24px_rgba(0,0,0,0.06)]">
                  {p.qty <= 0 ? (
                    <span className="absolute left-3 top-3 rounded bg-rose-100 px-2 py-0.5 text-[10px] font-semibold text-rose-700">
                      Out of Stock
                    </span>
                  ) : !!p.discount ? (
                  ...
                  <div className="px-3 pb-3">
                    ...
                    {/* Order button */}
                       <button
                      type="button"
                      onClick={() => orderProduct(p.id, p.qty)}
                      disabled={!Number.isFinite(Number(p.qty)) || Number(p.qty) <= 0}
